...
@skip_empty_sheet
def beam_conns_table(beamGeom_wksh):
    beam_elmt = []

    conn_data = beamGeom_wksh.iloc[2:, [0, 15, 16, 17, 18, 19, 20, 36]]
    ...
    for i, start_index in enumerate(beam_elmt):
        end_index = conn_data.index[-1] if i == len(beam_elmt) - 1 else beam_elmt[i + 1] - 1  # Ending index
    ...
    # For the final index in beam_elmt, repeat the final non-null value until the end of conn_data
    final_index = beam_elmt[-1]
    ...
    beamConn_rcds = conn_data.loc[:, ["Product_ID", "Connection_Type", "Position_XYZ", "Depth", "Diameter", "Notes"]]
    beamConn_tuples = [tuple(row) for row in beamConn_rcds.itertuples(index=False, name=None)]

    # Insert query
    beamConn_insert = "INSERT INTO Beam_Connections (Product_ID, Connection_Type, Position_XYZ, Depth, Diameter, Notes) \
        VALUES (%s, %s, %s, %s, %s, %s)"
    
    return (beamConn_tuples, beamConn_insert)
