...
@skip_empty_sheet
def wall_conns_table(wallGeom_wksh):
    wall_elmt = []

    conn_data = wallGeom_wksh.iloc[2:, [0, 20, 21, 22, 23, 24, 25, 42]]
    ...
    for i, start_index in enumerate(wall_elmt):
        end_index = conn_data.index[-1] if i == len(wall_elmt) - 1 else wall_elmt[i + 1] - 1  # Ending index
    ...
    # For the final index in wall_elmt, repeat the final non-null value until the end of conn_data
    final_index = wall_elmt[-1]
    ...
    wallConn_rcds = conn_data.loc[:, ["Product_ID", "Connection_Type", "Position_XYZ", "Depth", "Diameter", "Notes"]]
    wallConn_tuples = [tuple(row) for row in wallConn_rcds.itertuples(index=False, name=None)]

    # Insert query
    wallConn_insert = "INSERT INTO Wall_Connections (Product_ID, Connection_Type, Position_XYZ, Depth, Diameter, Notes) \
        VALUES (%s, %s, %s, %s, %s, %s)"
    
    return (wallConn_tuples, wallConn_insert)
