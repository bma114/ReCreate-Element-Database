...
def slab_longReinf_table(slabLongRF_wksh):

    lngReinf_data = slabLongRF_wksh.iloc[2:, [0, 2, 8, 9, 10, 11, 16, 17, 18, 19, 20, 35]]
    ...
    # Create unqiue IDs
    zoneID = slabLongRF_wksh.iloc[2:, 0] + "_" + slabLongRF_wksh.iloc[2:, 1]
    layerID = zoneID + "_" + slabLongRF_wksh.iloc[2:, 7]

    # Create zone polygon coordinate array (H = horizontal, V = vertical) relative to chosen plane
    zone_cols = slabLongRF_wksh.iloc[2:, [3, 4, 5, 6]]
    ...
    layer_cols = slabLongRF_wksh.iloc[2:, [12, 13, 14, 15]]
    ...
    # Anchorage Type
    anchor_raw = slabLongRF_wksh.iloc[2:, [21]]
    ...
    slabLongRF_rcds = pd.concat([lngReinf_data, zoneID, layerID, zone_polygon, layer_linestr, anchor_type], axis=1)

    # Rearrange columns to match MySQL ordering
    slabLongRF_rcds = slabLongRF_rcds.loc[:, ["Reinf_ID", "Zone_ID", "Zone_Plane", "Zone_Coords", "Layer_ID", "Layer_Coords", "Layer_Start", 
                                              "Num_Bars", "Bar_Diameter", "Bar_Spacing", "Anchorage_Type", "Steel_Grade", 
                                              "Min_Cover", "Confidence_Margin", "Notes"]]
    slabLongRF_tuples = [tuple(row) for row in slabLongRF_rcds.itertuples(index=False, name=None)]

    # Insert query
    slabLongRF_insert = "INSERT INTO Slab_Long_Reinf (Reinf_ID, Zone_ID, Zone_Plane, Zone_Coords, Layer_ID, Layer_Coords, Layer_Start, \
    ...
    return (slabLongRF_rcds, slabLongRF_tuples, slabLongRF_insert)