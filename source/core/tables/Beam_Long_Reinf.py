...
def beam_longReinf_table(beamLongRF_wksh):

    lngReinf_data = beamLongRF_wksh.iloc[2:, [0, 2, 8, 9, 10, 11, 16, 17, 18, 19, 20, 35]]
    ...
    # Create unqiue IDs
    zoneID = beamLongRF_wksh.iloc[2:, 0] + "_" + beamLongRF_wksh.iloc[2:, 1]
    layerID = zoneID + "_" + beamLongRF_wksh.iloc[2:, 7]

    # Create zone polygon coordinate array (H = horizontal, V = vertical) relative to chosen plane
    zone_cols = beamLongRF_wksh.iloc[2:, [3, 4, 5, 6]]
    ...
    layer_cols = beamLongRF_wksh.iloc[2:, [12, 13, 14, 15]]
    ...
    # Anchorage Type
    anchor_raw = beamLongRF_wksh.iloc[2:, [21]]
    ...
    beamLongRF_rcds = pd.concat([lngReinf_data, zoneID, layerID, zone_polygon, layer_linestr, anchor_type], axis=1)

    # Rearrange columns to match MySQL ordering
    beamLongRF_rcds = beamLongRF_rcds.loc[:, ["Reinf_ID", "Zone_ID", "Zone_Plane", "Zone_Coords", "Layer_ID", "Layer_Coords", "Layer_Start", 
                                              "Num_Bars", "Bar_Diameter", "Bar_Spacing", "Anchorage_Type", "Steel_Grade", 
                                              "Min_Cover", "Confidence_Margin", "Notes"]]
    beamLongRF_tuples = [tuple(row) for row in beamLongRF_rcds.itertuples(index=False, name=None)]

    # Insert query
    beamLongRF_insert = """
    INSERT INTO Beam_Long_Reinf (Reinf_ID, Zone_ID, Zone_Plane, Zone_Coords, Layer_ID, Layer_Coords, Layer_Start, 
    ...
    ON DUPLICATE KEY UPDATE
        ...
        Zone_Plane        = VALUES(Zone_Plane),
        Zone_Coords       = VALUES(Zone_Coords),
        Layer_ID          = VALUES(Layer_ID),
        Layer_Coords      = VALUES(Layer_Coords),
        Layer_Start       = VALUES(Layer_Start),
        Num_Bars          = VALUES(Num_Bars),
        Bar_Diameter      = VALUES(Bar_Diameter),
        Bar_Spacing       = VALUES(Bar_Spacing),
        Anchorage_Type    = VALUES(Anchorage_Type),
        Steel_Grade       = VALUES(Steel_Grade),
        Cover_Depth       = VALUES(Cover_Depth),
        Confidence_Margin = VALUES(Confidence_Margin),
        Notes             = VALUES(Notes);
    """
    
    return (beamLongRF_rcds, beamLongRF_tuples, beamLongRF_insert)