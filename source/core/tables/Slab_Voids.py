...
@skip_empty_sheet
# Slab Voids table data formatting function
def slab_voids_table(slabGeom_wksh, slabGeom_rcds):

    void_indices = []
    void_array = []

    # Find all void indices (different to slab indices)
    for index, rows in slabGeom_wksh.iloc[2:].iterrows():
        if not pd.isnull(rows.iloc[13]) and rows.iloc[13] != 0:
            void_indices.append(index)

    # Add raw data to pd
    void_data = slabGeom_wksh.iloc[:, [0, 28]]
    ...
    # Fill all empty Product ID's with the previous ID (for slabs with more than one void)
    last_prodID = None
    ...
    for i in range(len(void_indices) - 1):
        ...
        for index in range(start_index, end_index):
            x = slabGeom_wksh.iloc[index, 14]
            y = slabGeom_wksh.iloc[index, 16]
            z = slabGeom_wksh.iloc[index, 15]
    ...
    slabVoid_rcds = pd.concat([void_data, void_coords, void_shell], axis=1)

    # Rearrange columns to match MySQL ordering
    slabVoid_rcds = slabVoid_rcds.loc[:, ["Product_ID", "Void_Coords_XYZ", "Void_Shell_XYZ", "Notes"]]

    # Drop all rows that have null coordinates (slabs with no voids)
    slabVoid_rcds.replace({"": np.nan, " ": np.nan}, inplace=True)
    slabVoid_rcds = slabVoid_rcds.dropna(subset=["Void_Coords_XYZ"])

    slabVoid_tuples = [tuple(row) for row in slabVoid_rcds.itertuples(index=False, name=None)]

    # Insert query
    slabVoid_insert = "INSERT INTO Slab_voids (Product_ID, Void_Coords_XYZ, Void_Shell_XYZ, Notes) \
            VALUES (%s, %s, %s, %s)"
    
    return (slabVoid_rcds, slabVoid_tuples, slabVoid_insert)